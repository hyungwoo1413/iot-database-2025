...
- SQL 기초
    ...
    - DML 중 SELECT
        ...
        - 쿼리 연습(정렬까지) : [SQL](./day02/db02_select_practice.sql)
...
- Visual Studio Code에서 MySQL 연동
    - 확장 > MySQL 검색
        ...
        - Weijan Chen이 개발한 Database Client 설치(추천)
            - 데이터베이스 아이콘 생성
        - Oracle에서 개발한 MySQL Shell for VS Code를 사용하지말것
    - Database Client
...
- SQL 기초
    - 기본 데이터형
        - 데이터베이스에는 엄청 많은 데이터형이 존재(데이터의 사이즈로 저장용량을 절약하기 위해서)
        - 주요 데이터형
            ...
            - Decimal(5~17) - Float보다 더 큰수 저장
            - Char(n) - n은 가변(1~255), 고정길이 문자열
                - 주의점! Char(10)에 Hello를 입력하면 **'Hello     '** 저장
            - Varchar(n) - n은 가변(1~65535), 가변길이 문자열
                - 주의점! Varchar(10)에 Hello를 입력하면 **'Hello'** 저장
            - LongText(최대 4GB) - 뉴스나 영화스크립트 저장
            - LongBlob(최대 4GB) - mp3, mp4 음악, 영화데이터 자체를 저장
            - Date(3) - 2025-02-27 저장
            - DateTime(8) - 2025-02-27 10:46:34 저장
    ...
    - DDL 중 CREATE
        ...
        ```
        - DDL문은 Workbench에서 마우스 클릭으로 많이 사용
        - 테이블 생성 후 확인
            1. 메뉴 Database > Reverse Engineer (데이터베이스를 ERD 변경) 클릭
            2. 연결은 패스
            3. Select Schema to RE 에서 특정 DB를 체크
            4. Execute 버튼을 클릭
            5. ERD를 확인
    ...
    - DML 중 INSERT, UPDATE, DELETE : [SQL](./day03/db02_dml_쿼리.sql)
        ...
        VALUES (값리스트);  -- VALUE는 SQL 표준이 아님(MySQL, PostgreSQL 등 에서만 사용가능)
...
- SQL 고급
    ...
    - 인덱스: [SQL](./day04/db04_sql고급_인덱스.sql)
        ...
        - 주의점
            ...
            - 테이블당 인덱스 개수는 5개 미만으로 생성할 것(너무 많으면 성능 저하)
            - 자주 변경되는 컬럼에는 인덱스 생성하지 말 것(성능 저하)
            - NULL 값이 많은 컬럼에 인덱스 생성하지 말 것(성능 저하)
        
- 데이터베이스 프로그래밍
    - 저장 프로시저 : [SQL](./day04/db05_저장프로시저1.sql)
        - 너무 많은 쿼리로 일을 처리해야할 때, 파이썬등 프로그램에서 구현하면 매우 복잡함
        - 저장 프로시저 하나로 프로그램 구현시 코드가 매우 짧아짐
        - 개발 솔루션화, 구조화해서 손쉽게 DB처리를 가능하게 하기 위해서 사용
...
- 데이터베이스 연동 프로그래밍
    ...
    - 파이썬 DB연동 웹(Flask) : [Python](./day05/index.py)
        - template 폴더내 html 저장

- 데이터모델링
    - 현실세게에 데이터처리내용을 디지털 환경에 일치시켜서 모델링
    - DB 생명주기
        1. 요구사항 수집 및 분석
        2. 설계
        3. 구현
...
## 6일차
- 데이터모델링 : [SQL](./day06/마당대학_CREATE_스키마.sql)
...
- 정규화 : [SQL](./day06/db03_정규화_이상현상.sql)
    ...
    - 함수 종속성 - 하나의 속성(A)이 다른 속성(B)을 결정지으면 A는 B의 결정자. A → B. A가 기본키가 됨
        - 완전함수종속 - 종속성에 불일치하는 속성들이 하나도 없는 경우
        - 이행적종속 - A → B, B → C 일때 A → C 가 되는 종속성
        - BCNF 해당하지 않는 경우 - A → C, B → C 일때 C → B 경우가 발생
    - 정규화 - 이상현상이 발생하지 않도록 릴레이션(개체, 테이블)을 분해하는 과정
        ...
        - BCNF정규화 - 함수종속성 A → B 가 성립시 모든 결정자 A가 후보키가 되는 정규화(무손실 분해)
...
- Workbench Tip
    ...
    - 메뉴 `Edit > Preferences > SQL Editor > SQL Execution > Limit Rows Count` 조절
